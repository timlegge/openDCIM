	...
	*/
	...
	function processChassis($dev,$dept,$row,$ca_result,$tpList,$cList){
		...
		foreach($childList as $child){
			...
			( $_POST['resolve'] == 'true' ? ($child->PrimaryIP != '' ? "\t<td>" . @gethostbyname($child->PrimaryIP) . "</td>" : "\t<td></td>" ) : $noop )
	...
	if(isset($_REQUEST['datacenterid'])){
		...
		$body="<table id=\"export\" class=\"display\">\n\t<thead>\n\t\t<tr>\n
			...
			( $_POST['resolve'] == 'true' ? "\t<th>" . __("Resolved IP") . "</th>" : $noop ) . 
		...
		foreach($result as $row){
			...
			if($row["ParentDevice"]=="0"){
				...
				\t<td>{$row["AssetTag"]}</td>
				\t<td>{$row["PrimaryIP"]}</td>" .
				( $_POST['resolve'] == 'true' ? ($row["PrimaryIP"] != '' ? "\t<td>" .@gethostbyname($row["PrimaryIP"]) . "</td>" : "\t<td></td>") : $noop )
